...
{	
	...
	// blinn phong的镜面反射计算方式：计算中程向量，然后计算中程向量和法线的点积
	vec3 midDir = normalize(viewDir + lightDir);// 中程向量计算方式,blinn-phong，更柔和一些
	float spec = pow(max(0,dot(norm,midDir)),specularExponent);

	// phong的计算镜面光照方式：计算反射向量，然后计算反射向量和视线向量的点积
	// vec3 reflectDir = reflect(-lightDir,norm); // 反射向量计算方式,glsl的reflect api取入射方向是从光源到顶点。 
