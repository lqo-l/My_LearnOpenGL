...
int main(int argc, char** argv)
{
	...
	std::cout << "texturePath2: " << texturePath2 << std::endl;
	...
	if (data) {
		...
		std::cout << "Failed to load texture:"<< texturePath2 << std::endl;
	}
	stbi_image_free(data);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		// 计算变换矩阵M= TRS. 后续用于M*vec = TRS*vec.
		// api的计算
		glm::mat4 trans = glm::mat4(1.0f); // I
		trans = glm::translate(trans, glm::vec3(0.5f, -0.5f, 0.0f)); // I * T
		trans = glm::rotate(trans, glm::radians(float(glfwGetTime())*10), glm::vec3(0.f, 0.f, 1.f)); // I * T * R
		trans = glm::scale(trans, glm::vec3(0.5f, 0.5f, 0.f)); // I * T * R * S = M
		glUniformMatrix4fv(glGetUniformLocation(shader.ID, "transform"), 1, GL_FALSE, glm::value_ptr(trans)); // 指针转换
		...
		glUniformMatrix4fv(glGetUniformLocation(shader.ID, "transform"), 1, GL_FALSE, &trans[0][0]); // 指针转换
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);
