...
int main()
{
	...
	ImGui_ImplGlfw_InitForOpenGL(window, true); // 这里imgui绑定的回调与自定义的相机cursor回调冲突覆盖，TODO：待解决
	...
	//glfwSetCursorPosCallback(window, mouse_callback); /// 鼠标指针回调 ，测试imgui，临时注释了
	//glfwSetScrollCallback(window, scroll_callback); /// 滚轮回调
	//glfwSetInputMode(window, GLFW_CURSOR, GLFW_CURSOR_DISABLED); /// 锁定光标
	glEnable(GL_DEPTH_TEST); /// 深度测试

	/// 着色器
	Shader objectShader("phong_view.vert", "object_phong_view.frag"); // 视图空间中计算phong物体着色器
	Shader lightShader("phong_world.vert", "light.frag"); // 光源着色器
